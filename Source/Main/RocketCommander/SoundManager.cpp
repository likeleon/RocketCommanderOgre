...
void SoundManager::Initialize(void)
{
	...
	result = system->setFileSystem(&fmodFileOpenCallback, 
		&fmodFileCloseCallback, 
		&fmodFileReadCallback, 
		&fmodFileSeekCallback,
		NULL,
		NULL,
		2048);
	if (result != FMOD_OK)
...
void SoundManager::Update(Ogre::SceneNode *listenerNode, Ogre::Real timeElapsed)
{
	int            channelIndex;
...
int SoundManager::CreateStream(const String &fileName)
{
	return CreateSound(fileName, SOUND_TYPE_2D_SOUND);
}

int SoundManager::CreateSound(const String &fileName)
{
	return CreateSound(fileName, SOUND_TYPE_3D_SOUND);
}

int SoundManager::CreateLoopedSound(const String &fileName)
{
	return CreateSound(fileName, SOUND_TYPE_3D_SOUND_LOOPED);
}

int SoundManager::CreateLoopedStream(const String &fileName)
{
...
int SoundManager::CreateSound(const String &fileName, SOUND_TYPE soundType)
{
	Archive *      fileArchive;
	FMOD_RESULT    result;
	FMOD::Sound *  sound;
...
void SoundManager::PlaySound(int soundIndex, SceneNode *soundNode, int *channelIndex, float volume)
{
...
int SoundManager::FindSound(const String &fileName, SOUND_TYPE soundType)
{
	int            vectorIndex;
	int            vectorCapacity;
