...
public:
	...
	int                  CreateSound(const String &fileName);         // single-shot 3D sound.  returns soundIndex
	int                  CreateStream(const String &fileName);        // single-shot 2D stream.  returns soundIndex
	int                  CreateLoopedSound(const String &fileName);   // looping 3D sound.  returns soundIndex
	int                  CreateLoopedStream(const String &fileName);  // looping 2D stream.  returns soundIndex

	int                  CreateSound(const String &fileName, SOUND_TYPE soundType);

	void                 PlaySound(int soundIndex, SceneNode *soundNode, int *channelIndex, float volume = 1.0f);
	void                 StopSound(int *channelIndex);
	int                  FindSound(const String &fileName, SOUND_TYPE soundType);    // returns soundIndex;
